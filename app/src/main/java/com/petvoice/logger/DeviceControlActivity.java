...
//    private long starttime = 0;
    ...
    class PrevSensorData{
        int sensorValueIdx = -1;
        float time = -0.05f;
        float[]  accel = {0.0f,0.0f,0.0f};
        float[] gyro = {0.0f,0.0f,0.0f};
        float[] compass = {0.0f,0.0f,0.0f};
        float temp = 0.0f;

        public void setData(int idx, float time, float[] accel, float[] gyro, float[] compass, float temp){
...
//            }
    ...
    public void dspReadCharacteristic(byte[] bytes) {
        if (bytes.length <21){
            Log.e("ERR:","Characteristic Lengith is more than 21");
            return;
        }

        int idx = bytes[0]&0xFF;
        if (prevSensorData.isBegin()){ // first recoding..
            prevSensorData.sensorValueIdx = idx;
        }

        float[] accel = Util.getAccel(bytes, 0*6 + 1);
        float[] gyro = Util.getGyro(bytes, 1 * 6 + 1);
        float[] compass = Util.getCompass(bytes, 2 * 6 + 1);
        float temp = Util.getTemperature(bytes);

        int idxInterval = prevSensorData.getInterval(idx);

        if ( saveCsv && idxInterval >1 ){
            for (int k = 0; k < idxInterval - 1; k++){
                prevSensorData.time += 0.05f;
                float[] t_accel = new float[3];
                float[] t_gyro = new float[3];
                float[] t_compass = new float[3];
                t_accel[0] = prevSensorData.accel[0] + ((accel[0] - prevSensorData.accel[0])/(float)idxInterval)*(k+1);
                t_accel[1] = prevSensorData.accel[1] + ((accel[1] - prevSensorData.accel[1])/(float)idxInterval)*(k+1);
                t_accel[2] = prevSensorData.accel[2] + ((accel[2] - prevSensorData.accel[2])/(float)idxInterval)*(k+1);

                t_gyro[0] = prevSensorData.gyro[0] + ((gyro[0] - prevSensorData.gyro[0])/(float)idxInterval)*(k+1);
                t_gyro[1] = prevSensorData.gyro[1] + ((gyro[1] - prevSensorData.gyro[1])/(float)idxInterval)*(k+1);
                t_gyro[2] = prevSensorData.gyro[2] + ((gyro[2] - prevSensorData.gyro[2])/(float)idxInterval)*(k+1);

                t_compass[0] = prevSensorData.compass[0] + ((compass[0] - prevSensorData.compass[0])/(float)idxInterval)*(k+1);
                t_compass[1] = prevSensorData.compass[1] + ((compass[1] - prevSensorData.compass[1])/(float)idxInterval)*(k+1);
                t_compass[2] = prevSensorData.compass[2] + ((compass[2] - prevSensorData.compass[2])/(float)idxInterval)*(k+1);
                Util.exportData(Util.formatTime(prevSensorData.time), t_accel, t_gyro, t_compass, temp,false);
            }
        }
...
//                Util.exportData(recordingTime, accel, gyro, compass);
            Util.exportData(Util.formatTime(prevSensorData.time+0.05f), accel, gyro, compass, temp,true);
        }
        prevSensorData.setData(idx, prevSensorData.time+0.05f, accel, gyro, compass, temp);
    }
